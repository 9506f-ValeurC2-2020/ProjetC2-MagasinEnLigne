...
class OrdersAdapter(private val orders: ArrayList<Order>, private val isAdmin: Boolean = false) :
    ...
    inner class OrderViewHolder(override val containerView: View) :
        ...
        fun bindOrder(order: Order, position: Int) {
            ...
            with(order) {
                ...
                itemView.tv_cost.text = mContext.getString(R.string.cost, "$cost")
                if (isAdmin) {
                    itemView.tv_id.text = mContext.getString(R.string.id, id)
                    itemView.tv_from_client.text =
                        mContext.getString(R.string.from_client, fromClient)
                    itemView.tv_to_merchant.text =
                        mContext.getString(R.string.to_merchant, toVendeur)
                    itemView.tv_id.show()
                    itemView.tv_from_client.show()
                    itemView.tv_to_merchant.show()
                }
            }
        }
    }
